% hw3.pl
% Max Dupler
% Portions of this code were generated by ChatGPT, an AI language model developed by OpenAI.
% Code snippets were provided by ChatGPT for the purpose of assisting the user in solving the specified problem.
% The final implementation may have been modified by the user.

% Specific problems where chatgpt was used are problem 8 and 9 for parts 
% of code generation and fixing errors
...
getCommon(STATE1, STATE2, PLACELST) :-
    findall(Place, location(_, Place, STATE1, _, _, _), PlaceLst1),
    findall(Place, location(_, Place, STATE2, _, _, _), PlaceLst2),
    intersection(PlaceLst1, PlaceLst2, Places),
    sort(Places, PLACELST).
...
% Assuming the PLACELST is holding the result from the previous query
% ------------------------------------------------
